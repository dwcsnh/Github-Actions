...
const makePaginator = <T extends PaginatorOptions<any>>(opts: Partial<T> = {}): Paginator<typeof opts> => {
  ...
  const getFieldConfig = (field: string | FieldConfig): FieldConfig =>
  ...
  const getPagination = (req: Request): { page: number; pageSize: number } => {
    const pageField = getFieldConfig(fields.page);
    const pageSizeField = getFieldConfig(fields.pageSize);
  ...
  const getSorter = (req: Request): { field: string; direction: 'asc' | 'desc' }[] => {
    const sortField = getFieldConfig(fields.sort);
  ...
  const getFilter = (
    ...
    const filterField = getFieldConfig(fields.filter);
